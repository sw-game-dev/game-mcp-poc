...
./scripts/build.sh
```
This builds both the backend binary and the frontend WASM assets.

### Run Production Server
```bash
./scripts/serve.sh
```
This runs the production build of the backend server, which also serves the frontend static files.

## Prerequisites
- Rust (2024 edition)
- cargo
- trunk (for WASM builds - will be auto-installed by scripts)
- wasm-bindgen-cli (will be auto-installed by scripts)
